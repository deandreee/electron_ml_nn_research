...
export const linregSplitRSI = (candlesActual: Candle[], fnGetInd: fnGetInd, pctChange: number[], name: string) => {
  const indArr = candlesActual.map(fnGetInd);
  const indArrSplit = indArr.map((x, i) => (x <= 30 ? [x, pctChange[i]] : null)).filter(x => !!x);

  if (indArr.length !== pctChange.length) {
    throw new Error(`linreg length not equal => ${name} | ${indArr.length} vs ${pctChange.length}`);
  }

  const avgPct = round2(meanBy(indArrSplit, x => x[1]));
